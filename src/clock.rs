    ...
    Event: Debug + TimeExchange,
{
    ...
    fn process(&mut self, event: &Event) -> Self::Audit {
        ...
        let time_diff_secs = time_event_exchange.signed_duration_since(lock.time_exchange_last).num_seconds().abs();
...
mod tests {
    ...
    fn test_historical_clock_process() {
        ...
        let plus_ms = |ms: i64| time_base.checked_add_signed(TimeDelta::milliseconds(ms)).unwrap();
    ...
    fn test_historical_clock_time_delta_calculation() {
        ...
        assert!(time_2 > time_1, "Historical clock time should increase with wall clock");
        ...
        assert!(delta_ms >= 95 && delta_ms <= 105, "Historical clock time delta outside expected range");
    }
}
